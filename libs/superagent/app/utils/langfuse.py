...
class LangfuseHandler(BaseCallbackHandler):
    ...
    def end_trace(
        ...
        trace_map: Optional[Dict[str, List[str]]] = None,  # noqa: F841
    ) -> None:
    ...
    def on_event_start(
        ...
        **kwargs: Any,
        ...
        CBEvent(event_type, payload=payload, id_=event_id)
        ...
        if event_type == CBEventType.LLM:
            ...
            self._lf_object_map[event_id] = parent.generation(
                CreateGeneration(
                    name="OpenAI Chat",
                    prompt=prompt,
                    completion=response_text,
                    model=str(model_parameters.get("model", "")),
            ...
            self._lf_object_map[event_id] = parent.event(
                CreateEvent(name=exception_name, metadata=str(exception))
            )
        return event_id
